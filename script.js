...
const products = [
    ...
    { 
        ...
        mol: "C₈H₁₅NaO₈", purity: ">99%", desc: "A white to cream-colored powder used as a thickener, stabilizer, and binder. Known for high viscosity and water solubility.", 
...
function viewDetails(id) {
    ...
    if (p.cat === "Colours") {
        ...
        specs.push({ label: "Function", value: p.func }, { label: "Key Features", value: p.features || "N/A" }, { label: "Appearance", value: p.appearance || "N/A" }, { label: "Formula", value: p.mol });
        if (p.weight) specs.push({ label: "Mol. Weight", value: p.weight });
        if (p.density) specs.push({ label: "Density", value: p.density });
        if (p.melting) specs.push({ label: "Melting Point", value: p.melting });
        specs.push({ label: "Grade", value: p.grade }, { label: "Purity", value: p.purity });
    }
    else { 
    ...
    if (list) {
        list.innerHTML = cart.map(c => `
            <div class="flex justify-between text-sm bg-white p-3 rounded-xl border border-slate-100">
                <div class="flex flex-col"><span class="font-bold text-slate-800">${c.name}</span><span class="text-[10px] text-slate-400 uppercase font-black">${c.stock}</span></div>
...
function renderSidebar() {
    ...
    let html = `
        <button onclick="filterProducts('All')" class="w-full text-left px-4 py-3 rounded-xl hover:bg-slate-50 transition font-bold text-sm mb-4 flex items-center justify-between group border border-transparent">
    ...
    html += categories.map(cat => {
        ...
        return `
            <div class="category-group border-b border-slate-50 last:border-0 pb-1">
                <button onclick="toggleSidebarDropdown(this, '${cat}')" class="w-full flex items-center justify-between px-4 py-3 rounded-xl hover:bg-slate-50 transition group text-left">
                    <span class="text-[11px] font-black text-slate-500 group-hover:text-[#004b8d] uppercase tracking-tighter">${cat} <span class="text-slate-300 font-medium ml-1">(${catItems.length})</span></span>
                ...
                <div class="dropdown-content hidden pl-2 pr-2 py-2 space-y-1">
                    <button onclick="filterProducts('${cat}')" class="w-full text-left py-1.5 px-3 text-[10px] font-black uppercase text-[#1a7139] hover:bg-green-50 rounded-lg transition">View All ${cat}</button>
                    ${catItems.map(p => `<button onclick="filterSingleProduct(${p.id})" class="w-full text-left py-1.5 px-3 text-[11px] text-slate-500 hover:text-[#004b8d] hover:bg-blue-50 rounded-lg transition truncate">${p.name}</button>`).join('')}
                </div>
            </div>
        `;
    }).join('');
